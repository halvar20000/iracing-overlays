...
class RaceLogger(SDKPoller):
    ...
    def __init__(self):
        ...
        # behind moves up by 1) are counted too.
        #
        # IMPORTANT — these two are CUMULATIVE and stay that way. They
        # answer "how much position churn did this driver go through",
        # which is what driver_of_the_day.py scores as its `overtakes`
        # component. They are NOT the +/- column: a driver who loses a
        # place and takes it straight back adds 1 here to each counter,
        # but his +/- is back to 0. See _pos_delta() / self._grid below
        # for that number.
        ...
        # Starting-grid baseline for the +/- column. NET vs the grid, never
        # cumulative: pos_delta = grid_slot - current_position. Owned by
        # GridBaseline (iracing_sdk_base.py), which captures the grid once
        # per race, resets on session change, and leaves a car without a
        # baseline (→ blank cell) rather than inventing one when the logger
        # was attached mid-race.
    ...
    # ----- grid baseline / net +/- ---------------------------------------
    def _update_grid_baseline(self) -> None:
        """Keep the starting-grid baseline fresh. Cheap after capture."""
        self._grid.update(self.ir)

    def _pos_delta(self, car_idx: int, position: int) -> int | None:
        """NET places gained (+) or lost (-) versus the STARTING GRID.

        grid_slot - current_position, and nothing else. It does not
        accumulate: a driver who is shuffled back one place at turn 1 and
        repasses reads 0 again, and reads 0 every subsequent time that
        same place changes hands. Returns None when this car has no known
        grid slot (late joiner, or the logger attached mid-race) so the UI
        can leave the cell blank instead of showing a made-up number.
        """
        return self._grid.delta(car_idx, position)
    ...
    def _update_overtake_counters(self) -> None:
        ...
        These counters are CUMULATIVE by design and feed the Driver of
        the Day "overtakes" score. The +/- column does NOT use them — see
        _pos_delta().
        """
        if self._log_fp is None:
            return
        ...
        for d in self._log_session_meta.get("drivers", []):
            ...
            if cur_lap > prev:
                ...
                event = {
                    ...
                    # and scored by driver_of_the_day.py.
                    "overtakes":   self._overtakes_made.get(idx, 0),
                    "overtaken":   self._overtakes_against.get(idx, 0),
                    # Starting grid slot + NET places vs it at this lap.
                    # grid_pos is written on every lap event so post-race
                    # tools (DotD, render_race) can measure gains against
                    # the real grid instead of guessing from lap 1.
                    "grid_pos":    self._grid.grid_pos.get(idx),
                    "pos_delta":   self._pos_delta(
                        idx, int(ovr_pos[idx]) if idx < len(ovr_pos) else 0),
                }
    ...
    def _build_drivers_state(self) -> list[dict]:
        ...
        for d in drivers_raw:
            ...
            out.append({
                ...
                # Cumulative churn — feeds Driver of the Day, NOT the +/-.
                "overtakes":   self._overtakes_made.get(idx, 0),
                "overtaken":   self._overtakes_against.get(idx, 0),
                # NET vs the starting grid — this is what the +/- shows.
                "grid_pos":    self._grid.grid_pos.get(idx),
                "pos_delta":   self._pos_delta(idx, pos),
    ...
    def _read_snapshot(self) -> dict:
        ...
        # Track the starting grid on EVERY tick, not just while logging.
        # It has to run before the green flag (so a green-flag sample is
        # still an option) and in sessions we don't log, so that by the
        # time the race starts the baseline is already known.
        self._update_grid_baseline()
...
def share_standings_data():
    ...
    for r in state:
        safe.append({
            ...
            "grid_pos":     r.get("grid_pos"),
            "pos_delta":    r.get("pos_delta"),
    ...
    <div class="panel">
      ...
      <div class="drivers" id="drivers">
        <div class="drv-row head">
          ...
          <div style="text-align:right;" title="Net positions gained / lost vs the starting grid">+/−</div>
...
function render(d) {
  ...
  if (!drivers.length) {
    ...
    for (const r of drivers) {
      ...
      // +/- column: NET places gained or lost versus the STARTING GRID.
      // Not a running tally — a driver who drops a place at turn 1 and
      // repasses is back on "=", and stays on "=" however often that
      // place changes hands again. Blank when we have no grid slot for
      // this car (late joiner, or the logger attached mid-race).
      // The cumulative pass counts are still there, in the tooltip.
      const pd = r.pos_delta;
      const ot = r.overtakes || 0;
      const ag = r.overtaken || 0;
      const otTitle = r.grid_pos
        ? `started P${r.grid_pos} · ${ot} gained / ${ag} lost during the race`
        : `${ot} gained / ${ag} lost during the race`;
      let otCell;
      if (pd == null) {
        otCell = `<span class="zero" title="${otTitle}">—</span>`;
      } else if (pd === 0) {
        otCell = `<span class="zero" title="${otTitle}">=</span>`;
      } else if (pd > 0) {
        otCell = `<span class="up" title="${otTitle}">+${pd}</span>`;
      } else {
        otCell = `<span class="down" title="${otTitle}">−${Math.abs(pd)}</span>`;
      }
...
<div class="wrap">
  ...
  <div class="card" style="padding: 0;">
    <div class="row head">
      ...
      <div class="col-ot" style="text-align:right;" title="Net positions gained / lost vs the starting grid">+/&minus;</div>
...
function fmtPosDelta(pd, gridPos) {
  // NET positions gained/lost versus the STARTING GRID — green up arrow,
  // red down arrow, "=" when the driver is exactly where he started.
  // This never accumulates: drop a place and take it back and you are on
  // "=" again, no matter how many times that repeats. A grey dash means
  // we have no grid slot for this car (late joiner / attached mid-race),
  // which is deliberately different from "held position".
  const t = gridPos ? ` title="started P${gridPos}"` : '';
  if (pd == null)  return `<span class="ot flat">—</span>`;
  if (pd > 0) return `<span class="ot up"${t}>▲${pd}</span>`;
  if (pd < 0) return `<span class="ot down"${t}>▼${-pd}</span>`;
  return `<span class="ot flat"${t}>=</span>`;
}
      ...
      for (const r of drivers) {
        ...
        html += `
          <div class="row ${rowCls}">
            ...
            <div class="col-ot">${fmtPosDelta(r.pos_delta, r.grid_pos)}</div>
